...
CHROMIUM_LANGS="af am ar az bg bn bs ca cs da de el en-GB en-US es-419 es et fa fi fil
	fr gu he hi hr hu id it ja ka kk km kn ko lo lt lv mk ml mn mr ms my nb nl pl
	pt-BR pt-PT ro ru si sk sl sq sr-Latn sr sv sw ta te th tr uk ur uz vi zh-CN zh-TW"

inherit brave chromium-2 desktop eapi9-pipestatus pax-utils unpacker verify-sig xdg

DESCRIPTION="Brave Origin, a minimalist build of the Brave Web Browser"
HOMEPAGE="https://brave.com/origin/"

# The stable channel installs its launcher and man page under a -stable
# suffix, while the pre-release channels use the package name unchanged.
# Brave publishes amd64 and arm64 .deb packages for stable, but arm64
# appears only sporadically for beta and nightly, so those are amd64-only.
# Keep BRAVE_ARCHES in sync with KEYWORDS below.
if [[ ${PN} == brave-origin ]]; then
	MY_PN=${PN}-stable
	BRAVE_ARCHES="amd64 arm64"
else
	MY_PN=${PN}
	BRAVE_ARCHES="amd64"
fi
...
LICENSE="MPL-2.0"
SLOT="0"
# Keep literal: eix parses ebuilds textually and would otherwise pick up the
# wrong branch of a conditional assignment.
KEYWORDS="-* ~amd64"
...
pkg_pretend() {
	...
	local arch
	for arch in ${BRAVE_ARCHES}; do
		use ${arch} && return
	done
	die "${PN} is only available for: ${BRAVE_ARCHES}"
}
...
src_install() {
	...
	# The stable channel ships a generic brave-origin.1.gz symlink.
...
pkg_postinst() {
	...
	elog "Brave Origin installs its launcher as /usr/bin/${MY_PN}."
